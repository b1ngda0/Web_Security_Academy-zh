...
参数化查询可用于不受信任的输入作为查询中显示为数据的任何情况，包括`WHERE`子句和`INSERT`或`UPDATE`语句中的值。它们不能用于处理查询其他部分中不可信任的输入，例如表或列名或`ORDER BY`子句。将不受信任的数据放入查询的这些部分应用程序功能需要采用不同的方法，例如将允许的输入值列入白名单，或者使用不同的逻辑来传递所需的行为。
